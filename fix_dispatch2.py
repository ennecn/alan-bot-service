#!/usr/bin/env python3
import paramiko

client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect('192.168.21.111', username='fangjin', password='YYZZ54321!')

new_dispatch = r'''#!/bin/bash
# claude-code-dispatch.sh - Dispatch a task to Claude Code
# Uses a detached runner script to survive SSH disconnection

export PATH=/usr/local/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/opt/homebrew/lib/node_modules/.bin:/usr/bin:/bin:/usr/sbin:/sbin:$PATH
[ -f "$HOME/.bash_profile" ] && source "$HOME/.bash_profile" 2>/dev/null
[ -f "$HOME/.zshrc" ] && source "$HOME/.zshrc" 2>/dev/null

PROMPT=""
TASK_NAME="task-$(date +%s)"
WORKDIR="/Users/fangjin/claude-workspace/alin"
MAX_TURNS=20

while [[ $# -gt 0 ]]; do
    case $1 in
        -p|--prompt) PROMPT="$2"; shift 2;;
        -n|--name) TASK_NAME="$2"; shift 2;;
        -w|--workdir) WORKDIR="$2"; shift 2;;
        -t|--max-turns) MAX_TURNS="$2"; shift 2;;
        *) shift;;
    esac
done

if [ -z "$PROMPT" ]; then
    echo '{"error": "prompt required (-p \"your prompt\")"}'
    exit 1
fi

mkdir -p "$WORKDIR"

# Write task meta
cat > "$WORKDIR/task-meta.json" << TMEOF
{
    "name": "$TASK_NAME",
    "prompt": $(echo "$PROMPT" | jq -Rs .),
    "workdir": "$WORKDIR",
    "max_turns": $MAX_TURNS,
    "started_at": "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
}
TMEOF

# Write prompt to temp file
PROMPT_FILE="/tmp/claude-prompt-$$.txt"
echo "$PROMPT" > "$PROMPT_FILE"

# Write self-contained runner script
RUNNER_FILE="/tmp/claude-runner-$$.sh"
cat > "$RUNNER_FILE" << RUNEOF
#!/bin/bash
export PATH=/usr/local/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/opt/homebrew/lib/node_modules/.bin:/usr/bin:/bin:/usr/sbin:/sbin:\$PATH
[ -f "\$HOME/.bash_profile" ] && source "\$HOME/.bash_profile" 2>/dev/null
[ -f "\$HOME/.zshrc" ] && source "\$HOME/.zshrc" 2>/dev/null

PROMPT=\$(cat "$PROMPT_FILE")
rm -f "$PROMPT_FILE"

cd "$WORKDIR" && \\
/opt/homebrew/bin/claude -p "\$PROMPT" \\
    --max-turns $MAX_TURNS \\
    --output-format json \\
    --dangerously-skip-permissions \\
    2>&1 | tee "$WORKDIR/task-output.txt"

rm -f "\$0"
RUNEOF
chmod +x "$RUNNER_FILE"

# Use launchctl to run detached from this session (survives SSH disconnect)
PLIST_FILE="/tmp/claude-task-$$.plist"
cat > "$PLIST_FILE" << PEOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.claude-code.task.$$</string>
    <key>ProgramArguments</key>
    <array>
        <string>$RUNNER_FILE</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
    <key>StandardOutPath</key>
    <string>/tmp/claude-task-$$.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/claude-task-$$.log</string>
    <key>EnvironmentVariables</key>
    <dict>
        <key>HOME</key>
        <string>/Users/fangjin</string>
        <key>PATH</key>
        <string>/usr/local/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/opt/homebrew/lib/node_modules/.bin:/usr/bin:/bin:/usr/sbin:/sbin</string>
    </dict>
</dict>
</plist>
PEOF

launchctl load "$PLIST_FILE" 2>/dev/null
# Clean up plist after a delay (launchctl already read it)
( sleep 5 && rm -f "$PLIST_FILE" ) &

cat << EOF
{
    "status": "dispatched",
    "task_name": "$TASK_NAME",
    "workdir": "$WORKDIR",
    "max_turns": $MAX_TURNS,
    "message": "Claude Code running via launchctl. Results -> /Users/fangjin/claude-code-results/latest.json"
}
EOF
'''

sftp = client.open_sftp()
with sftp.file('/Users/fangjin/claude-code-dispatch.sh', 'w') as f:
    f.write(new_dispatch)
sftp.close()

stdin, stdout, stderr = client.exec_command('chmod +x /Users/fangjin/claude-code-dispatch.sh')
stdout.read()

print("[OK] Updated dispatch script with launchctl approach")
client.close()
