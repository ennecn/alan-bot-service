...
const server = http.createServer((req, res) => {
  if (req.method === 'POST' && req.url === '/v1/messages') {
    ...
    res.writeHead(200);
    res.end(JSON.stringify({ status: 'ok', target: 'llm-gateway:8080', client: 'Alin', fix: 'stopReason' }));
  } else {
    res.writeHead(404);
    res.end('Not Found');
  }
...
server.listen(8022, '127.0.0.1', () => {
  console.log('[Proxy] Alin ready on http://127.0.0.1:8022 -> LLM Gateway (with stopReason fix)');
});
