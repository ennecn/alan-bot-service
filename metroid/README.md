...
**v0.2.0** — 128 tests passing, 10 test files

| 模块 | 状态 | 说明 |
|------|------|------|
| Memory Engine | ✅ | STM/LTM, 关键词检索, 异步编码, 遗忘曲线 |
| Identity Engine | ✅ | 角色卡加载, 灵魂锚点, 可变特质 |
| World Engine | ✅ | ST 世界书完整兼容 (selective logic, position/depth, probability) |
| Emotion Engine | ✅ | PAD 模型, 规则分析, 风格提示, 惯性+恢复 |
| Growth Engine | ✅ | 行为模式检测, 漂移边界, 审计追踪 |
| Prompt Compiler | ✅ | 双模式组装 (classic ST / enhanced Metroid) |
| CLI v0.2 | ✅ | 10 个命令, 状态行 |
| HTTP Adapter | ✅ | REST API, OpenClaw bot 集成 |
| Social Engine | ❌ | 多 agent 感知 (计划中) |
...
│   │   └── schema.sql        # 数据库 Schema v2
│   ├── engines/
│   │   ├── identity/         # 身份引擎
│   │   ├── memory/           # 记忆引擎 (Store/Retriever/Encoder/Forgetter)
│   │   ├── world/            # 世界引擎 (ST 兼容)
│   │   ├── emotion/          # 情绪引擎 (PAD 模型)
│   │   └── growth/           # 成长引擎
│   ├── importers/
