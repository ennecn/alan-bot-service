...
function init() {
  ...
  // V8: Push social events to WS clients
  metroid.onNewPost((agentId, post) => {
    wsBroadcast(agentId, { type: 'new_moment', post });
  });

  metroid.onNewReaction((postId, reaction) => {
    // Broadcast to all connected clients (social is public)
    for (const client of wsClients) {
      wsSend(client, { type: 'new_reaction', postId, reaction });
    }
  });
...
route('POST', '/debug/relationship/:id/:userId', async (req, res, { id, userId }) => {
  const agent = metroid.getAgent(id);
  if (!agent) return error(res, 'agent not found', 404);
  const body = await readBody(req);
  const result = metroid.setRelationship(id, userId, {
    attachment: body.attachment !== undefined ? Number(body.attachment) : undefined,
    trust: body.trust !== undefined ? Number(body.trust) : undefined,
    familiarity: body.familiarity !== undefined ? Number(body.familiarity) : undefined,
  });
  json(res, { ok: true, relationship: result });
});
...
// === V8: Social / Moments API ===

route('GET', '/moments', (req, res) => {
  const url = new URL(req.url!, `http://${req.headers.host}`);
  const limit = parseInt(url.searchParams.get('limit') ?? '20');
  const feed = metroid.getSocialFeed(Math.min(50, limit));
  json(res, { posts: feed });
});

route('GET', '/agents/:id/moments', (req, res, { id }) => {
  const agent = metroid.getAgent(id);
  if (!agent) return error(res, 'agent not found', 404);
  const url = new URL(req.url!, `http://${req.headers.host}`);
  const limit = parseInt(url.searchParams.get('limit') ?? '20');
  const posts = metroid.getAgentMoments(id, Math.min(50, limit));
  json(res, { posts });
});

route('POST', '/moments', async (req, res) => {
  const body = await readBody(req);
  if (!body.content) return error(res, 'content required');
...
route('POST', '/moments/:id/like', async (req, res, { id }) => {
  const body = await readBody(req);
  const actorId = body.userId || 'user-api';
  const actorType = body.actorType || 'user';
  const reaction = metroid.addSocialReaction(id, actorId, actorType, 'like');
  if (!reaction) return error(res, 'already liked or post not found');
  json(res, { reaction });
});

route('POST', '/moments/:id/comment', async (req, res, { id }) => {
  const body = await readBody(req);
  if (!body.content) return error(res, 'content required');
  const actorId = body.userId || 'user-api';
  const actorType = body.actorType || 'user';
  const reaction = metroid.addSocialReaction(id, actorId, actorType, 'comment', sanitize(body.content, 300));
  if (!reaction) return error(res, 'comment failed');
  json(res, { reaction });
});

route('GET', '/moments/:id/reactions', (_req, res, { id }) => {
  const reactions = metroid.getSocialReactions(id);
  json(res, { reactions });
});

route('GET', '/agents/:id/social-credit', (_req, res, { id }) => {
  const agent = metroid.getAgent(id);
  if (!agent) return error(res, 'agent not found', 404);
  const credit = metroid.getSocialCredit(id);
  json(res, { credit });
});

route('POST', '/debug/social-tick/:id', async (_req, res, { id }) => {
  try {
    await metroid.triggerSocialTick(id);
    json(res, { ok: true });
  } catch (err: any) {
    error(res, err.message ?? 'social tick failed');
  }
});
