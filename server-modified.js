...
import { routeRequest, getRouterStatus, getFallbackStatus, resetTier, resetAllTiers } from './router.js';
...
const routes = {
  ...
  'GET /api/fallback/status': (req, res) => {
    const status = getFallbackStatus();
    sendJson(res, 200, status);
  },

  'POST /api/fallback/reset/:provider/:model': (req, res, params) => {
    const success = resetTier(params.provider, decodeURIComponent(params.model));
    if (success) {
      sendJson(res, 200, { success: true, message: 'Tier reset successfully' });
    } else {
      sendJson(res, 404, { success: false, error: 'Tier not found or already available' });
    }
  },

  'POST /api/fallback/reset-all': (req, res) => {
    resetAllTiers();
    sendJson(res, 200, { success: true, message: 'All tiers reset successfully' });
  },
